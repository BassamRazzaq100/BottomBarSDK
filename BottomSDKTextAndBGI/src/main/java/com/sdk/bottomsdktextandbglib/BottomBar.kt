...
import com.sdk.bottomsdktextandbglib.ext.d2p
import com.sdk.bottomsdktextandbglib.model.BottomBarItem

class BottomBar @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = R.attr.BottomBarStyle
    ...
    @DrawableRes
    private var _barIndicatorDrawable: Int = -1
    ...
    var barIndicatorDrawable: Int?
        @DrawableRes get() = _barIndicatorDrawable
        set(@DrawableRes value) {
            _barIndicatorDrawable = value ?: -1
            //paintIndicator.color = value
            invalidate()
        }
    ...
    private fun obtainStyledAttributes(attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        try {
            barBackgroundColor = typedArray.getColor(
                R.styleable.BottomBar_backgroundColor,
                barBackgroundColor
            )
            barIndicatorColor = typedArray.getColor(
                R.styleable.BottomBar_indicatorColor,
                barIndicatorColor
            )
            barIndicatorRadius = typedArray.getDimension(
                R.styleable.BottomBar_indicatorRadius,
                barIndicatorRadius
            )
            barSideMargins = typedArray.getDimension(
                R.styleable.BottomBar_sideMargins,
                barSideMargins
            )
            barCornerRadius = typedArray.getDimension(
                R.styleable.BottomBar_cornerRadius,
                barCornerRadius
            )
            barCorners = typedArray.getInteger(
                R.styleable.BottomBar_corners,
                barCorners
            )
            itemPadding = typedArray.getDimension(
                R.styleable.BottomBar_itemPadding,
                itemPadding
            )
            itemTextColor = typedArray.getColor(
                R.styleable.BottomBar_textColor,
                itemTextColor
            )
            itemTextSize = typedArray.getDimension(
                R.styleable.BottomBar_textSize,
                itemTextSize
            )
            itemIconSize = typedArray.getDimension(
                R.styleable.BottomBar_iconSize,
                itemIconSize
            )
            itemIconMargin = typedArray.getDimension(
                R.styleable.BottomBar_iconMargin,
                itemIconMargin
            )
            itemIconTint = typedArray.getColor(
                R.styleable.BottomBar_iconTint,
                itemIconTint
            )
            itemIconTintActive = typedArray.getColor(
                R.styleable.BottomBar_iconTintActive,
                itemIconTintActive
            )
            itemActiveIndex = typedArray.getInt(
                R.styleable.BottomBar_activeItem,
                itemActiveIndex
            )
            itemFontFamily = typedArray.getResourceId(
                R.styleable.BottomBar_itemFontFamily,
                itemFontFamily
            )
            itemAnimDuration = typedArray.getInt(
                R.styleable.BottomBar_duration,
                itemAnimDuration.toInt()
            ).toLong()
            itemMenuRes = typedArray.getResourceId(
                R.styleable.BottomBar_menu,
                itemMenuRes
            )
            barIndicatorDrawable = typedArray.getResourceId(
                R.styleable.BottomBar_indicatorDrawable,
                barIndicatorDrawable ?: -1
            )
        } catch (e: Exception) {
            e.printStackTrace()
        } finally {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (barIndicatorDrawable != -1) {
            val bitmap = BitmapFactory.decodeResource(resources, barIndicatorDrawable!!)
            try {
                canvas.drawBitmap(
                    bitmap,
                    Rect(
                ...
                Log.d("BitmapError", "Done")
            } catch (ex: Exception) {
                Log.d("BitmapError", ex.toString())
            }
        } else {
            canvas.drawRoundRect(
                rect,
     ...
     *
     * Just call [BottomBar.setOnItemSelectedListener] to override [onItemSelectedListener]
     *
     * @sample
     ...
     * Created by Vladislav Perevedentsev on 29.07.2020.
     *
     * Just call [BottomBar.setOnItemReselectedListener] to override [onItemReselectedListener]
     *
     * @sample
